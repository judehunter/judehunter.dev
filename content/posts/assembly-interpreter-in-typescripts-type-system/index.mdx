...
Here's where it's a bit different. For the evaluation, instead of simply putting the value into the desired register, we use the FullAdder to apply addition.

```ts
type EvalAddImmediateInstr<T, Ctx extends Context> =
  T extends AddImmediataeInstr<
    infer Rd,
    infer Rn,
    infer imm
  >
