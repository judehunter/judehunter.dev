...
createDate: '2022-11-14T18:50:56.511Z'
updateDate: '2022-11-14T18:50:56.511Z'
thumbnail: /media/deepmind-Snqm29dhfOk-unsplash.jpg
description: 'TypeScript''s type system is crazy advanced. We can even build entire programming languages on the type level.'
tags:
  - typescript
  - compilers
  - assembly
...
And thus, [ts-asm](https://github.com/judehunter/ts-asm) was born. The only thing I ever wanted was to calculate the fibonacci sequence in TypeScript's type system using Assembly. I can sleep peacefully now.
...
Our program is simply an array of indexed instruction objects. As I mentioned multiple times now (hope you're paying attention), the `pc` Program Counter register will contain the current instruction to be processed.
...
This pattern of incrementing `pc` will be so universal that we should use abstract it with a utility type.
...
```ts
// when translated to JS, this would like:
...
We can't do much with just the `MOV` instruction. We should support a plethora of arithmetic and logical operations, as well as implement branching so that we can write programs with loops and conditional statements.

### Addition

Let's start small --- the `ADD` instruction. Just like `MOV`, it has an `immediate` and `register` variant, which determines the type of the third and last operand.
...
There's still more instructions to be added. There is something we need to deal with first though.

# The Little Big Problem

As the interpreter complexity kept growing, I encountered a problem I hoped wouldn't pop up in this project. It's that same problem that plagued me in my original project of implementing a full programming language inside of TypeScript's type system.
...
<Idea>
  I attempted this patch-fix when I was working on the original full programming language project. For some reason I couldn't get it to work. Then, suddenly, it started working on its own. I suspect VS Code was doing some caching behind the scenes.
</Idea>
...
// pops the value and places is in r0
POP {r0}
```
...
<Idea>
  You might have noticed that the label got its own instruction index. Normally, after assembling, the label gets removed and any `Branch` instructions will point to the next instruction. For this project however, it's easier to treat labels as no-ops.
</Idea>
...
Since `B` is literally syntax sugar over `MOV pc, ...`, our interpreter doesn't need to know any new functionality has been added. We can simply desugar the former into the latter.
...
Branching by itself isn't all that useful. To write if statements, we need a way to branch only when a condition is met. Let's add two more instructions, `Compare and Branch If Zero` and<br/>`Compare and Branch If Not Zero`. They will read the value from the specified register to determine whether or not to branch.
...
This time, we do need to make changes to the interpreter, as there's no way to desugar this into any set of other instructions. Since the two instructions are similar, I used just one object type `BranchIf` to denote them.
...
<Note>
The Link Register is used as a way to store the Program Counter temporarily before branching so that you can branch back (return) to it later.
</Note>
...
The desugaring and evaluation of `BL` and `BR` is trivial.

# Fin

Once a daunting task, now a distant memory. You can check the whole codebase for the project in the official repository, [ts-asm](https://github.com/judehunter/ts-asm).

On the topic of programming languages and compiler theory, you might also want to check out [The journey of Queso, my programming language](/blog/the-journey-of-queso-my-programming-language).