...
```ts
type EvalAddImmediateInstr<T, Ctx extends Context> =
  T extends AddImmediateInstr<
    infer Rd,
    infer Rn,
    infer imm
  >
...
type TwosComplement<T> = FullAdder<
  OnesComplement<T>,
  '00000001'
>['result'];

// add the minuend and the opposite of the subtrahend
type Subtract<A, B> = FullAdder<A, TwosComplement<B>>;
```
```ts
InterpretProgram<`
  MOV r0, #00000110
...
```ts
type EvalLdrImmediateInstr<T, Ctx extends Context> =
  T extends LdrImmediateInstr<
...
```ts
type EvalStrImmediateInstr<T, Ctx extends Context> =
  T extends StrImmediateInstr<
    infer Rt,
    infer Rn,
    infer imm
  >
    ...
    ? {
        ...
        registers: Overwrite<
          ...
          Record<
            Rm,
            Ctx['memory'][
              // notice that this addition happends
...
```ts
type EvalLabelInstr<T, Ctx extends Context> =
  T extends LabelInstr<string>
    ? {
        memory: Ctx['memory'];
        registers: Overwrite<
          Ctx['registers'],
